...
| 문제 | 고민 | 해결 |
|---|---|---|
| **로직 분산** — 필터링·통계 계산이 컴포넌트 안에 흩어지면 재사용·테스트가 어려움 | UI와 계산 로직을 어떻게 분리하나 | 계산 로직을 **Custom Hook**(`useFilteredTodos`, `useTodoStats`)으로 분리해 컴포넌트는 표현에만 집중 |
| **상태 전파** — 여러 페이지·컴포넌트가 같은 할 일 목록을 공유 (prop drilling) | 전역 상태를 어떻게 관리하나 | **Context API**(`TodoContext`)로 전역 상태를 한 곳에 두고 필요한 곳에서 구독 |
